function waveletmodel(yic,yis,Fs,fh,ff,fd,ffl){
    ...
    let a = Math.PI**2*fd**2/2;
    let L = fl.length;
    let c = isr(cn,0,1/Math.sqrt(a));
    let r = 1; let Tfg = 2*Math.PI/fd;
    tg = [];
    ...
    let Tf = []; let NH = []; let H_t = []; let H_ts = [];
    for (let l=1;l<=L;l++){
        console.log("Harmonic " + l + ":");
        for (let j=0;j<=tg.length-1;j++){
            gc[j] = Math.exp(-1*(Math.PI*fd*tg[j])**2/2)*Math.cos(2*Math.PI*fl[l-1]*tg[j])*P[j];
            gs[j] = Math.exp(-1*(Math.PI*fd*tg[j])**2/2)*Math.sin(2*Math.PI*fl[l-1]*tg[j])*P[j];
        }
        
        [yfc,yfs] = conv(yic,yis,gc,gs);
        [Gc,Gs] = fft(gc,gs); //fftttttttttttttttttttttttttttttttt
        let Ga=[];
        ...
        //[afc,afs] = conv(afc,afs,[1/6,1/6,1/6,1/6,1/6,1/6],[0,0,0,0,0,0]); //experiment smoothing
    ...
    let yrec = reconstruct(afz,t,ff)
    return [yrec,yo,afz,t,Gc,Gs,tg]
}
function iht(fh){
    let K=fh.length-1;
    let r = [];
...
function conv(t1c,t1s,t2c,t2s){
    ...
    let [Y1c,Y1s] = fft(y1c,y1s); //fftttttttttttttttttttttttttttttttt
    let [Y2c,Y2s] = fft(y2c,y2s); //fftttttttttttttttttttttttttttttttt
    let Y3c = dots(dotm(Y1c,Y2c),dotm(Y1s,Y2s));
    let Y3s = dota(dotm(Y1c,Y2s),dotm(Y1s,Y2c));
    [y3c,y3s] = ifft(Y3c,Y3s); //ifftttttttttttttttttttttttttttttttt
